...
public extension APIServiceType {
    ...
    func request(
        ...
        let url = try generateURL(withResourcePath: resourcePath)
        return await Self.request(url: url, httpMethod: httpMethod, parameterData: parameterData)
    }

    /// Generate a `URLRequest` injected with the proper headers and query items.
    /// - Parameters:
    ///   - resourcePath: The absolute path for creating the full endpoint path.
    ///   - httpMethod: The HTTP request method. Defaults to POST.
    ///   - queryItems: A dictionary of information to be converted to query items as part of the request.
    ...
    func request(
        ...
        var url = try generateURL(withResourcePath: resourcePath)
        url.append(queryItems: queryItems.toQueryItems())
        return await Self.request(url: url, httpMethod: httpMethod)
    }
...
private extension APIServiceType {

    func generateURL(withResourcePath resourcePath: String) throws -> URL {
...
private extension JSON {
    ...
    func toQueryItems() -> [URLQueryItem] {
        return self.compactMap {
            guard let name = queryString(from: $0), let value = queryString(from: $1) else {
                return nil
            }
